...
export default function DefaultMenuPage({ params: { lang } }) {
    ...
    function getMenuData() {
        ...
        // GET THE MENU DATA
        axios.get(`${process.env.API_URL}/get/chiff/menu`, { headers: { Authorization: `Bearer ${token}` } })
            .then(response => {
                // MENU
    ...
    function deleteDay() {
        ...
        // DELETE THE DAY
        axios.delete(`${process.env.API_URL}/delete/chiff/menu/day?date=${selectedDayToDelete?.date}`, { headers: { Authorization: `Bearer ${token}` } })
            .then(response => {
                // TOAST
    ...
    return (
        <div
            ...
            dir={lang === 'en' ? 'ltr' : 'rtl'}
        >
            <h3 className={'mb-5'}>{lang === 'en' ? 'Default Menu' : 'القائمة الافتراضية'}</h3>
            <DataTable
                dir={lang === 'en' ? 'ltr' : 'rtl'}
                value={menu || []}
                paginator
                ...
                header={lang === 'en' ? 'DEFAULT MENU' : 'القائمة الافتراضية'}
