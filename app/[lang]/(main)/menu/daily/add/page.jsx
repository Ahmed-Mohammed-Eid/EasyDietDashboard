...
export default function AddDailyMenuPage({ params: { lang } }) {
    ...
    function addDailyMenu() {
        ...
        // ADD DAILY MENU
        axios.post(`${process.env.API_URL}/add/menu/day`, {
            date,
            ...
            .then(response => {
                // TOAST
                toast.success(lang === 'en' ? 'Daily menu added successfully' : 'تمت إضافة القائمة اليومية بنجاح');
                // GET THE MENU DATA
                getMenuData();
            })
            .then(data => {
    ...
    return (
        <div className={'card mb-0'} dir={lang === 'en' ? 'ltr' : 'rtl'}>
            <h5>{lang === 'en' ? 'Add Daily Menu' : 'إضافة قائمة يومية'}</h5>
            <hr />
